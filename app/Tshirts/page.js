...
const Tshirts = () => {
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        ...
        const tshirts = data.filter((p) => p.category === "T-Shirts");
        setProducts(tshirts);
      } catch (err) {
  ...
  if (!products.length) {
    return <div className="text-center py-20 text-lg">No T-shirts found.</div>;
  }

  return (
    <div className="min-h-screen bg-gray-100 py-10 px-5">
      <h1 className="text-3xl font-bold text-center mb-10">T-Shirts Collection</h1>
