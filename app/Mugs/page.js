...
const Mugs = () => {
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        ...
        // Filter only mugs
        const mugs = data.filter((p) => p.category === "Mugs");
        setProducts(mugs);
      } catch (err) {
  ...
  if (!products.length) {
    return <div className="text-center py-20 text-lg">No mugs found.</div>;
  }

  return (
    <div className="min-h-screen bg-gray-100 py-10 px-5">
      <h1 className="text-3xl font-bold text-center mb-10">Mugs Collection</h1>
